...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        if edge_weight1 is not None:
            edge_weight1 = edge_weight1.to(device_ids['encoder1'])
        if edge_weight2 is not None:
            edge_weight2 = edge_weight2.to(device_ids['encoder2'])

        z1 = self.encoder1(x1.to(device_ids['encoder1']), edge_index1.to(device_ids['encoder1']), edge_weight1)
        z2 = self.encoder2(x2.to(device_ids['encoder2']), edge_index2.to(device_ids['encoder2']), edge_weight2)
        ...
        z2n = self.encoder2(*self.corruption(x2.to(device_ids['encoder2']), edge_index2.to(device_ids['encoder2']), edge_weight2))
...
def main():
    ...
    for dataname in datasets:
        ...
        for i in range(nm_trials):
            ...
            encoder_model = Encoder(encoder1=gconv1, encoder2=gconv2, augmentor=(aug1, aug2), hidden_dim=512, device_ids)
