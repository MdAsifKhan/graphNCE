...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None, test=False):
        ...
        #z2 = torch.einsum('k n d, k -> n d', z2.to(self.device_ids['projector']), coeff.to(self.device_ids['projector']))
        ...
        #z2n = torch.einsum('k n d, k -> n d', z2n, coeff.to(self.device_ids['encoder2']))
...
def train(encoder_model, contrast_model, data, optimizer):
    ...
    loss = 0.
    coeff = [1. for i in range(z.shape[0])]
    for scale in range(z.shape[0]):
        loss += coeff[scale] * contrast_model(h=z[scale], g=g[scale], hn=zn[scale])
...
def test(encoder_model, data):
    ...
    result = LREvaluator()(z.mean(0), data.y, split)
    return result
...
def main():
    ...
    data_eps = {'PubMed':1e-3, 'Cora':1e-3, 'Citeseer':1e-4}
    data_scales = {'PubMed': 8, 'Cora':16, 'Citeseer':8}
    nm_trials = 1
    diffusion = 'wavelet'
